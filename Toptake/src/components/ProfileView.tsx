...
import { getTodayPrompt } from '@/lib/promptService';
...
const ProfileView: React.FC<ProfileViewProps> = ({ userId, username }) => {
  ...
  const loadGlobalPrompt = async () => {
    try {
      ...
      setGlobalPrompt("What's one controversial opinion you hold that most people would disagree with?");
    }
  };
  ...
  const loadUserData = async () => {
    try {
      ...
      const takesWithPromptData = takes?.map((take, index) => {
        const dayNumber = takes.length - index;
